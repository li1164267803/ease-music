...
> ⚠️ **项目状态：开发中，尚无可用版本。** C1（工程基座与核心播放能力）、C2（离线缓存）、C3（播放历史）、C6（插件音源，含榜单与推荐歌单浏览）已实现完毕，正在真机验证阶段。当前阶段只在 Android 上开发与验收，iOS 侧留待 Android 功能全部完成后集中验收一次。下方标注为「待启动」的能力尚未实现。
...
| **C6**  | 插件音源系统（**仅 Android**，见下）· 榜单与推荐歌单浏览           | 已实现，iOS 侧待验证；专辑、艺人与外部歌单导入待启动 |
