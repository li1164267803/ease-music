...
- **来源可扩展**：新增内置来源时，只在 `src/sources` 下加一个目录并注册，其余各层不动。
...
注册方向是**插件推向注册表**（`registerSource` / `unregisterSource`），而不是注册表去拉插件——因此 `src/sources` 完全不知道插件的存在。这与本地文件、远程 URL 这类内置来源不同：内置来源编译期就在数组里；插件的数量与标识运行时才确定，**一个插件即一个来源**。
