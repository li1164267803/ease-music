 ...
 * - **应用自己为这首曲目产生的派生物一并清掉**：iOS 导入时复制进沙箱的那份副本、
 *   离线缓存下载的音频、提取出来落盘的封面、它的播放记录、以及歌词缓存。它们不是
 *   用户的资产，曲目记录消失后再无从界面触达，留着只会占存储、或在界面上留下点不响的条目。
 *
 * 缓存必须在删除曲目行**之前**清：`track_cache` 的外键是 ON DELETE CASCADE，
 * 曲目一删记录就没了，那时再想找缓存文件的路径已经无从查起。播放历史与歌词缓存
 * 没有这个问题——它们各只有一行数据、没有对应的文件，交给 CASCADE 删掉即可；
 * 历史这里只需在删完之后发一次广播，让「最近播放」把它去掉。
 */
